...
/*
 * Create a dir for writing, or if it exists, test if it
 * can be opened in write mode.
 *
 * Returns
 *	FM_DIR if the file exists and is writable,
 *	FM_DIR_CREATE if the file had to be created,
 *	FM_FILE if the path leads to a file, or
 *	FM_ERROR if the object is not a writable dir.
 */
int filemapper_t::test_dir_create(const char *syspath)
{
	int exists = (probe(syspath) == FM_DIR);
...
//			log_printf(DLOG, "  Is a directory.\n");
			...
			log_printf(WLOG, "mkdir: %s\n", syspath);
			if (::mkdir(syspath, 0))
				return FM_DIR_CREATE;
		}
//		else
